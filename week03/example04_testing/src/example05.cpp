...
void vertexJF(vector<GLfloat> &v, vector<GLfloat> &color, int i, int j)
{
    v.push_back (points[i][j][0]);
    v.push_back (points[j][j][1]);
    v.push_back (points[i][j][2]);
    v.push_back (1.0f);
    v.push_back (1.0f);
    v.push_back (1.0f);
}
void DrawGLScene(vector<GLfloat> &v,vector<GLfloat> &color){
    for (int x = 0; x < 45; x++) {
        for (int y = 0; y < 45; y++) {
            points[x][y][0]=float((x/5.0f)-4.5f)+0.046f;
            points[x][y][1]=float((y/5.0f)-4.5f)+0.06f;
            points[x][y][2]=float(sin((((x/8.0f)*40.0f)/360.0f)*3.141592654*4.0f));
//            cout<< points[x][y][0] << ",";
//            cout<< points[x][y][1] << ",";
//            cout<< points[x][y][2] << ",";
        }
        ...
        for (int x = 0; x < 44; x++) {
            for (int y = 0; y < 44; y++) {
                vertexJF(v,color,x,y);
                vertexJF(v,color,x,y+1);
                vertexJF(v,color,x+1,y+1);
                vertexJF(v,color,x+1,y);
            }
        }
    }
...
int main() {
    ...
    vector<GLfloat> flagVertex;
    vector<GLfloat> flagColor;
    DrawGLScene(flagVertex,flagColor);
//    DrawGLSquare();
//    concat_int(Buffer,Sbuffer,bufferS,allBuffer);

    cout <<flagVertex.size() <<endl;
    for(int i = 0; i < flagVertex.size(); i++)
    {
        std::cout << flagVertex[i]<<";";
    }
    ...
    glBufferData(GL_ARRAY_BUFFER, flagVertex.size()* sizeof(float) + flagColor.size()* sizeof(float), NULL, GL_STATIC_DRAW);

    glBufferSubData(GL_ARRAY_BUFFER,0,flagVertex.size()*sizeof(GLfloat),&flagVertex[0]);
    glBufferSubData(GL_ARRAY_BUFFER, flagVertex.size()* sizeof(float), flagColor.size()* sizeof(float), &flagColor[0]);
