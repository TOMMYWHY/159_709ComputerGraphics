...
void DrawGLScene(vector<GLfloat> &v,vector<GLfloat> &color){
    for (int x = 0; x < 45; x++) {
        for (int y = 0; y < 45; y++) {
            points[x][y][0]=float((x/55.0f)-4.5f)+0.046f;
            points[x][y][1]=float((y/100.0f)-4.5f)+0.06f;
...
void DrawGLScene(){
    for (int x = 0; x < 30; x++) {
        for (int y = 0; y < 30; y++) {
            points[x][y][0]=float((x/5.0f)-4.5f)+2.451f;
            points[x][y][1]=float((y/5.0f)-4.5f)+4.1f;
...
//        cout << Buffer[i] << ",";
    ...
    glBufferData(GL_ARRAY_BUFFER,(45408)*sizeof(GLfloat), Buffer, GL_STATIC_DRAW);
    ...
    // View Matrix
    float viewMatrix[16];
    float viewPosition[3] = { 0.0f,  0.0f,  9.0f};
