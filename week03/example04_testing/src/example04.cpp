...
/*void pointInit()
{
    ...
    for(int x=0; x<p; x++)
    {
        ...
        {
            // 向表面添加波浪效果
            points[x][y][0] = double((x / 5.0f));
            points[x][y][1] = double((y / 5.0f) );
            points[x][y][2] = 1.9f;
            cout <<points[x][y][0]<<",";
            cout <<points[x][y][1]<<",";
            cout <<points[x][y][2]<<"; ";
...
void DrawGLScene(vector<float> &vertex)
{
    int const p = 50;
//    vector<float> vertex;
    float points[ p ][ p ][3];
    for(int x=0; x<p; x++)
    {
        ...
        for(int y=0; y<p; y++)
        {
            ...
            points[x][y][1]=float(((y/5.0f))+0.06)*0.056;
            points[x][y][2]=float(sin((((x/8.0f)*40.0f)/360.0f)*3.141592654*4.0f))*0.1;

        }
        cout <<"  " <<endl;
    }
    for( int x = 0; x < p-1; x++ )               // 沿 X 平面 0-44 循环(45点)
    {
        for( int y = 0; y < p-1; y++ )           // 沿 Y 平面 0-44 循环(45点)
        {
...
//        cout << Buffer[i] << ",";
    ...
    glBufferData(GL_ARRAY_BUFFER,Buffer.size()*sizeof(GLfloat)*sizeof(GLfloat), &Buffer[0], GL_STATIC_DRAW);
...
//        // ----------------------------------------
        ...
        for (int i = 0; i < Buffer.size(); i++) {
            if(i%3 ==2){
//                Buffer[i] = waving;
                Buffer[i] = Buffer[i+1]*sin(0.8*timeValue+0.8* Buffer[i+1]);
                cout <<Buffer[i] <<" ; ";
            }
        }
        ...
        glBindVertexArray(VAO);
//        glDrawArrays(GL_LINE_LOOP, 0, 6075);
//        glDrawArrays(GL_POINTS, 0, 6075);
        glDrawArrays(GL_LINE_STRIP, 0, 6075);
        glBindVertexArray(0);
