...
glm::vec3 cubePositions[] = {
    ...
    glm::vec3 lightPos(0.0f, .3f, 3.0f);
...
//LightDirectional * light = new LightDirectional(glm::vec3(5.0f, 3.5f, 3.0f), glm::vec3(glm::radians(45.0f),glm::radians(45.0f),0));
//LightPoint * light = new LightPoint(glm::vec3(1.0f, 1.0f, -2.0f),glm::vec3(1.0f,1.0f,1.0f));
LightSpot * light = new LightSpot(lightPos,  glm::vec3(glm::radians(90.0f),0,0), glm::vec3(1.0f,1.0f,1.0f));
// ----- mouse function-------//
float lastX,lastY;
   ...
   /* glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)0);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for( int i = 0; i < 10; i++)
        {
            ...
            // 平行光 // 点光
            glUniform4f(glGetUniformLocation(shaderProgram->ID,"u_Light_Postion"),light->Position.x,light->Position.y,light->Position.z,0.0f );
            glUniform4f(glGetUniformLocation(shaderProgram->ID,"lightColor"), light->Color.x, light->Color.y, light->Color.z,1.0f );
            glUniform4f(glGetUniformLocation(shaderProgram->ID,"u_light_Dir"), light->Direction.x, light->Direction.y, light->Direction.z,0.0f );
            ...
            glUniform1i(glGetUniformLocation(shaderProgram->ID, "u_texture_Map"), 0); // 手动设置
