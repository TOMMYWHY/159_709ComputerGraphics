   ...
   /* glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)0);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for( int i = 0; i < 10; i++)
        {
            ...
            glUniform4f(glGetUniformLocation(shaderProgram->ID,"ambinetColor"), 0.1f,0.1f,0.1f,1.0f );

            glUniform4f(glGetUniformLocation(shaderProgram->ID,"u_Light_Postion"), lightPos.x,lightPos.y,lightPos.z,lightPos.w );
            glUniform4f(glGetUniformLocation(shaderProgram->ID,"lightColor"), 1.5f,1.5f,1.5f,1.0f );
...
//        cout <<"u_camera_Postion:"<<camera->Position.x <<","<< camera->Position.y <<","<< camera->Position.x <<" ; "<<endl;
        ...
        glm::mat4 lampModelMatrix = glm::translate(glm::mat4(1.0f),glm::vec3(lightPos));
