...
Model::Model(string directory,string model_name,GLuint shadeID,float Scale,  float Pos_y) {
...
void Model::read_obj() {
    cout <<this->Directory + this->Model_name<<endl;
    ifstream f;
...
//            cout <<"temp_pos : " <<temp_pos.x<< " " <<temp_pos.y<< " " <<temp_pos.z  <<endl;
        ...
        if(file_obj[i][0]=="vt"){
            ...
            /*if(!file_obj[i][3].empty()){
                cout <<"file_obj[i][3] has value"<< endl;
                temp_tex.z = stringToNum<double>(file_obj[i][3]);
            }*/
            /*temp_tex.y = stringToNum<double>(file_obj[i][1]); //long double
            temp_tex.x = stringToNum<double>(file_obj[i][2]);*/
            v_t_vn.all_vt.push_back(glm::vec4(temp_tex.x,1-temp_tex.y,0 ,0.0f));
//            v_t_vn.all_vt.push_back(glm::vec4(temp_tex.x,temp_tex.y,0 ,0.0f));
...
void Model::get_node_index() {
    ...
    if(this->nodes_num == 1){
        Node node;
        split(obj_file, node_groups, "\n");
        cout <<"single node :" <<node_groups.size()<<endl;
        node.line_count = node_groups.size();
...
void Model::processNode(Node &node) {
    ...
    for (int i = node.v_lineStartIndex; i < node.f_lineEndIndex; i++)
    {
        if(file_obj[i][0]=="usemtl"){
            ...
            cout <<"node.usemtl:"<<node.usemtl<< endl;
        }
        if(file_obj[i][0]=="f"){
            node.face_amount++;
            int vertexIndex[4],UVIndex[4],normalIndex[4];
            /*cout <<"file_obj[i].size():"<<file_obj[i].size() << ": "<<file_obj[i][0] <<" "
            <<file_obj[i][1]<<" "
            <<file_obj[i][2]<<" "
            <<file_obj[i][3] << "- "
            <<file_obj[i][4] << " "
...
//                cout <<"ka:" <<  node.node_texture.Ka[0] <<" " <<   node.node_texture.Ka[1] << " "<< node.node_texture.Ka[2]<<endl;
            ...
            if(trim(values[j]) == "Ns"){
                node.node_texture.Ns = stringToNum<double>( trim(values[j+1]) );
            }
...
void Model::Draw(Node &node) {
    ...
    glUniform1f(glGetUniformLocation(ShadeID, "uni_Ns"), node.node_texture.Ns);
