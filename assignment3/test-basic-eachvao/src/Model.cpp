...
//        cout << "node: " << i<<" : " <<  nodes[i].usemtl<<endl;
...
//            cout << "file_obj : " <<file_obj[i][0] << " "<<file_obj[i][1] << " "<<file_obj[i][2]  << " " <<file_obj[i][3] << " "<<endl;
...
//                        cout <<"temp_nor : " <<temp_nor.x<< " " <<temp_nor.y<< " " <<temp_nor.z  <<endl;
    ...
    for (int i = 0; i < this->v_t_vn.all_v.size(); i++) {
//        cout << "all v  :"<< this->v_t_vn.all_v[i].x<<" "<< this->v_t_vn.all_v[i].y  <<endl;
    }
};
...
//            temp_indexes.push_back(glm::ivec3(index_count,index_count+2,index_count+3));
//            temp_indexes.push_back(glm::ivec3(index_count,index_count+3,index_count+1));
    ...
    for (int i = 0; i < temp_buffer.size()/3; i=i+3) {
//        cout <<temp_buffer[i].x<< " " <<temp_buffer[i].y<< " " <<temp_buffer[i].z<< " " <<temp_buffer[i].w  <<endl;
//        cout <<temp_buffer[i+1].x<< " " <<temp_buffer[i+1].y<< " " <<temp_buffer[i+1].z<< " " <<temp_buffer[i+1].w  <<endl;
//        cout <<temp_buffer[i+2].x<< " " <<temp_buffer[i+2].y<< " " <<temp_buffer[i+2].z<< " " <<temp_buffer[i+2].w  <<endl;
    }

    for (int i = 0; i < temp_indexes.size(); i++) {
//        cout <<temp_indexes[i].x<< " " <<temp_indexes[i].y<< " " <<temp_indexes[i].z  <<endl;
//        cout <<temp_buffer[i+1].x<< " " <<temp_buffer[i+1].y<< " " <<temp_buffer[i+1].z<< " " <<temp_buffer[i+1].w  <<endl;
//        cout <<temp_buffer[i+2].x<< " " <<temp_buffer[i+2].y<< " " <<temp_buffer[i+2].z<< " " <<temp_buffer[i+2].w  <<endl;
    }
...
//    this->total_indexes.insert(this->total_indexes.end(),temp_indexes.begin(),temp_indexes.end());
//    this->total_buffer.insert(this->total_buffer.end(),temp_buffer.begin(),temp_buffer.end());
...
void Model::Draw(Node &node) {
    ...
    if(!node.node_texture.map_Kd_path.empty()){
        ...
        glUniform4f(glGetUniformLocation(ShadeID, "Ka"), node.node_texture.Ka[0],node.node_texture.Ka[1],nodes[0].node_texture.Ka[2],0.f);
    }
    if(!node.node_texture.map_Ks_path.empty()){
        glActiveTexture(GL_TEXTURE0 + texture_amount);
        ...
        glUniform4f(glGetUniformLocation(ShadeID, "Ks"), node.node_texture.Ks[0],node.node_texture.Ks[1],nodes[0].node_texture.Ks[2],0.f);
    }
    if(!node.node_texture.bump_path.empty()){
        glActiveTexture(GL_TEXTURE0 + texture_amount); //normal
