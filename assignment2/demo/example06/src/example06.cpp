...
//    const float radius = 33.8;
    ...
    for (int i = 0; i < buffer.size(); i++) {
        std::cout <<"buffer:"<<glm::to_string(buffer[i]).c_str() <<std::endl;
    }
	// ----------------------------------------
...
//	 GLuint program = goraudProgram;
	...
	//TODO stride 8??? why not 4？vert_Normal？？
...
//	float modelThetaY = 0.0f;
	float modelThetaY = glm::radians(-35.0f);
...
//	modelMatrix = glm::rotate(modelMatrix, modelThetaY, glm::vec3(0.0f, 1.0f, 0.0f));
	modelMatrix = glm::rotate(modelMatrix, modelThetaY, glm::vec3(0.0f, 0.0f, 1.0f));
//	modelMatrix = glm::rotate(modelMatrix, modelThetaX, glm::vec3(0.0f, 0.0f, 1.0f));

//    std::cout <<"modelMatrix:"<<glm::to_string(modelMatrix).c_str() <<std::endl;
    ...
    glm::vec3 viewUp      (0.0f,  1.0f,  0.0f); //world up
...
//	projectionMatrix = glm::perspective(glm::radians(10.0f), 1.0f, 0.2f, 10.0f);
...
//		viewPosition.z = cos(glfwGetTime()) * 5.0f;
        ...
        viewPosition.x = sin(45.0f) * 5.0f;
        viewPosition.z = cos(45.0f) * 5.0f;
