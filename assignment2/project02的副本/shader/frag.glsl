...
void main()
{
	vec4 Ka = texture(material_texture, frag_UV.xy);
//	vec4 Ka = vec4(1.0f, 1.0f, 1.0f, 0.1f);
//	vec4 Kd = texture(material_diffuse, frag_UV.xy);
	vec4 Kd = Ka;
...
//	vec4 uNormal = normalize(frag_Normal);
	vec4 uNormal = vec4(normalize(texture(texture_Normal, frag_UV.xy).rgb * 2.0f - 1.0f), 0.0f);
