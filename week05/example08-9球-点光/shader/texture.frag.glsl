...
uniform vec4 u_Light_position = vec4(0.0f, 0.0f, -10.0f, 1.0f);  //光源
uniform vec4 u_Light_color = vec4(2.0, 2.0f, 2.0f, 1.0f);  //光颜色， 光强
...
uniform vec4 material_ambient =  vec4(.1f, .1f, .1f, 0.1f);
...
void main () {
	...
	vec4 Ta =  ambientColor * texture(u_texture_Map, frag_UV.xy); // todo ambientColor

	vec4 Td = texture(u_texture_Map, frag_UV.xy) * max(dot(l, n), 0) * u_Light_color;

	float specularAmount = pow(max(dot(r, v), 0), material_shininess);
	vec4 Ts = (material_specular) * specularAmount * u_Light_color;

	//漫反射光即为光线跟反射点的法向量点乘
	//	pixel_Colour = Ta  + Td * attenuation + Ts * attenuation;
	//	pixel_Colour = Ta  + Td  + Ts ;

	pixel_Colour = ((Ta + (Td + Ts) * attenuation) * objColor);
