...
//    GLuint vaoSun = Create_VAO( vbo,ebo,buffer,indexes,program);
    ...
    // ----------------------- 光源 ------------------------------//
    GLuint sunProgram  = loadProgram("./shader/lightSource.vert.glsl",  NULL, NULL, NULL, "./shader/lightSource.frag.glsl");

    std::vector<glm::vec4> lightbuffer;
    std::vector<glm::ivec3> lightindexes;
	createTexturedCube(lightbuffer, lightindexes);
...
// 设置灯立方体的顶点属性（对我们的灯来说仅仅只有位置数据）
    glBufferData(GL_ARRAY_BUFFER, lightbuffer.size() * sizeof(glm::vec4), lightbuffer.data(), GL_STATIC_DRAW);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, lightindexes.size() * sizeof(glm::ivec3), lightindexes.data(), GL_STATIC_DRAW);
    // Get Position Attribute location (must match name in shader)
    GLuint lightposLoc = glGetAttribLocation(program, "vert_Position");

    // Get Normal Attribute location (must match name in shader)
    GLuint lightnorLoc = glGetAttribLocation(program, "vert_Normal");

    // Get Texture Attribute location (must match name in shader)
    GLuint lighttexLoc = glGetAttribLocation(program, "vert_UV");

    // Set Vertex Attribute Pointers
    glVertexAttribPointer(lightposLoc, 4, GL_FLOAT, GL_FALSE, 12 * sizeof(GLfloat), NULL);
    glVertexAttribPointer(lightnorLoc, 4, GL_FLOAT, GL_FALSE, 12 * sizeof(GLfloat), (GLvoid*)(4*sizeof(float)));
    glVertexAttribPointer(lighttexLoc, 4, GL_FLOAT, GL_FALSE, 12 * sizeof(GLfloat), (GLvoid*)(8*sizeof(float)));

    // Enable Vertex Attribute Arrays
    glEnableVertexAttribArray(lightposLoc);
    glEnableVertexAttribArray(lightnorLoc);
    glEnableVertexAttribArray(lighttexLoc);
    glBindVertexArray(0);
...
//        float theta;
        ...
        lampModelMatrix = glm::scale(lampModelMatrix, glm::vec3(0.5f));
...
//        glDrawArrays(GL_TRIANGLES, 0, 36);
        ...
        glDrawElements(GL_TRIANGLES, lightindexes.size() * 3, GL_UNSIGNED_INT, NULL);
